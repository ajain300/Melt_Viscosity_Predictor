...
def val_epochs(history, name,cut = 10, scaler = None, save = False, d_type = 'split'):
    ...
    try: 
        if type([1,2]) == type(history[0]):
            for hist in history:
                hist_val = hist[cut:]
                plt.plot(range(cut,len(hist)), hist_val)
        ...
        print(f"Could not make epoch plot for {name}")
    
    if save:
        plt.savefig(f'MODELS/{datetime.date.today()}_{d_type}/ANN_learning.png')
